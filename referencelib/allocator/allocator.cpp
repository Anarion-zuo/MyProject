...
static size_t pool_size() {
    return ((char *)allocator->end - (char *)allocator->begin);
}
...
//        *node = ob;
        node->next = (head_ptr *)((char *)node + step);
        node = node->next;
    }
    node->next = NULL;
    return (head_ptr *)((char *)node + step);
}
...
int allocate(char **item, size_t size) {
    ...
    if (psize >= size) {
        heads[index] = (head_ptr *)allocator->begin;
    ...
    if (psize != 0) {
        ...
        heads[pindex] = (head_ptr *)allocator->begin;
    ...
    new_size += roundup;
    // allocate new space
    struct head_ptr *new_space = (head_ptr *)malloc(new_size);
    if (new_space == NULL) {
        return stat_malloc_failed;
    }
    heads[index] = new_space;
    allocator->begin = make_list(new_space, index, 20);
    allocator->end = (char *)new_space + new_size;
    allocator->current_size += size;
...
void deallocate(void *item, size_t size) {
    ...
    struct head_ptr **heads = allocator->heads;
    struct head_ptr *ptr = (head_ptr *)item, *old_head = heads[index];
    heads[index] = ptr;
    ptr->next = old_head;
}